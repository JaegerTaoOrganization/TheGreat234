...
void MiniTree_Prim(MGraph G) {
	...
	for (i = 1; i < G.numvertextype; i++) {
		...
		putline(vex[adjvex[k]].x+3,vex[adjvex[k]].y+3,vex[k].x+3,vex[k].y+3);
...
void Mouse1(MGraph *G) {//点击功能模块，返回其对应序号
	...
	for (; is_run(); delay_fps(60))
	{
		...
		else if (X >= 280 && X <= 340 && Y >= 240 && Y <= 275 && msg.is_left()) {
			exit(0);
		}
	}
}
void Mouse2(MGraph *G) {//点击功能模块，返回其对应序号
	mouse_msg msg = { 0 };
	for (; is_run(); delay_fps(60))
	{
		//获取鼠标消息，这个函数会等待，等待到有消息为止
		...
		msg.wheel);*/
		mousepos(&X, &Y);
		if (X >= 815 && X <= 1000 && Y >= 70 && Y <= 85 && msg.is_left()) {
			CreateMGraph(G);			
		}
		else if (X >= 815 && X <= 1000 && Y >= 100 && Y <= 115 && msg.is_left()) {
			CreateRoad(G);
		}
		else if (X >= 815 && X <= 1000 && Y >= 130 && Y <= 145 && msg.is_left()) {
			DFSTraverse(*G);
		}
		/*else if (X >= 280 && X <= 445 && Y >= 280 && Y <= 315 && msg.is_left()) {
		return 4;
		...
		else if (X >= 815 && X <= 1000 && Y >= 230 && Y <= 245 && msg.is_left()) {
			putBackgroundimgsky();
			putMenu();
			Mouse1(G);
		}
